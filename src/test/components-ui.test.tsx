...
describe('Suite Complète : Composants d\'Interface UI, Alertes & Badges Visuels', () => {
  describe('1. Badges de Statut Matériel', () => {
    it('affiche le badge « En service » avec le style émeraude approprié', () => {
      const { getByTestId } = render(<StatutMaterielBadge statut="En service" />);
      const badge = getByTestId('statut-badge');
      expect(badge).toHaveTextContent('En service');
      expect(badge.className).toContain('text-emerald-700');
    });

    it('affiche le badge « En stock » avec le style bleu', () => {
      const { getByTestId } = render(<StatutMaterielBadge statut="En stock" />);
      const badge = getByTestId('statut-badge');
    ...
    it('affiche le badge « En panne » avec le style ambre', () => {
      const { getByTestId } = render(<StatutMaterielBadge statut="En panne" />);
      const badge = getByTestId('statut-badge');
    ...
    it('affiche le badge « Hors service » avec le style rose/rouge', () => {
      const { getByTestId } = render(<StatutMaterielBadge statut="Hors service" />);
      const badge = getByTestId('statut-badge');
  ...
  describe('2. Badges de Priorité Réclamation', () => {
    it('affiche la priorité Urgente avec le style d alerte rouge', () => {
      const { getByTestId } = render(<PrioriteBadge priorite="Urgente" />);
      const badge = getByTestId('priorite-badge');
      expect(badge).toHaveTextContent('Urgente');
      expect(badge.className).toContain('text-red-700');
    });

    it('affiche la priorité Haute avec le style orange', () => {
      const { getByTestId } = render(<PrioriteBadge priorite="Haute" />);
      const badge = getByTestId('priorite-badge');
    ...
    it('affiche la priorité Moyenne avec le style ambre', () => {
      const { getByTestId } = render(<PrioriteBadge priorite="Moyenne" />);
      const badge = getByTestId('priorite-badge');
    ...
    it('affiche la priorité Basse avec le style neutre', () => {
      const { getByTestId } = render(<PrioriteBadge priorite="Basse" />);
      const badge = getByTestId('priorite-badge');
  ...
  describe('3. Badges de Statut Ticket / Réclamation', () => {
    it('affiche le statut « Ouverte »', () => {
      const { getByTestId } = render(<StatutTicketBadge statut="Ouverte" />);
      expect(getByTestId('statut-ticket-badge')).toHaveTextContent('Ouverte');
    });

    it('affiche le statut « En cours »', () => {
      const { getByTestId } = render(<StatutTicketBadge statut="En cours" />);
      expect(getByTestId('statut-ticket-badge')).toHaveTextContent('En cours');
    });

    it('affiche le statut « Résolue »', () => {
      const { getByTestId } = render(<StatutTicketBadge statut="Résolue" />);
      expect(getByTestId('statut-ticket-badge')).toHaveTextContent('Résolue');
    });

    it('affiche le statut « Rejetée »', () => {
      const { getByTestId } = render(<StatutTicketBadge statut="Rejetée" />);
      expect(getByTestId('statut-ticket-badge')).toHaveTextContent('Rejetée');
    });
  });

  describe('4. Composant FormAlert (Messages de validation & alertes)', () => {
    it('rend correctement une alerte d erreur', () => {
      const { getByText } = render(<FormAlert type="error" message="Le champ référence interne est obligatoire." />);
      expect(getByText(/Le champ référence interne est obligatoire/i)).toBeInTheDocument();
    });

    it('rend correctement une alerte de succès', () => {
      const { getByText } = render(<FormAlert type="success" message="Matériel enregistré avec succès." />);
      expect(getByText(/Matériel enregistré avec succès/i)).toBeInTheDocument();
    });

    it('rend correctement un avertissement', () => {
      const { getByText } = render(<FormAlert type="warning" message="Attention : ce matériel est sans garantie active." />);
      expect(getByText(/Attention : ce matériel/i)).toBeInTheDocument();
    });
  });
});
